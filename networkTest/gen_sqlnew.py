...
def gen_update_sql(infos):
    ...
    sql_tpl = "update gb_task set target='${localip}' where name='${from_name}T${to_name}' and status=4;"
...
#    topos = ""
    ...
    for provider, provider_infos in infos.items():
        for region, region_list in provider_infos.items():
            for (from_name, from_clientid, from_ip,localip) in region_list:
                for (to_name, to_clientdid, to_ip,localip) in region_list:
                    if localip == None:
                        continue
                    if from_name == to_name:
                        continue
                    key_values = {"from_name" : from_name, "to_name" : to_name, "localip" : localip }
                    sql_result = Template(sql_tpl).substitute(key_values)
                    sql_results += sql_result + "\n"
#                    topos += "%s\t%s\n" %(from_name, to_name)

    output_file = "sql_result_update.%s" %(time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()))
